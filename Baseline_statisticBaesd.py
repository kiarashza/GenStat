...
The orginal implementation can be found at https://github.com/satyakisikdar/infinity-mirror/
"""
...
class _BTER(BaseGraphModel):
    ...
    def _gen(self, gname: str, gen_id: int) -> nx.Graph:
        ...
        graph_path = f'/local-scratch/kiarash/infinity-mirror/src/bter/{g.name}_{self.trial}.mat'
        ...
        matlab_code_path = f'/local-scratch/kiarash/infinity-mirror/src/bter/{matlab_code_filename}'

        print('\n'.join(matlab_code), file=open(matlab_code_path, 'w'))

        output_path = f'/local-scratch/kiarash/infinity-mirror/src/bter/{g.name}_{self.trial}_bter.mat'

        start_time = time()
        completed_process = sub.run(f'cd /local-scratch/kiarash/infinity-mirror/src/bter; cat {matlab_code_filename} | matlab -nosplash -nodesktop',
                                    shell=True,
...
class SBM(BaseGraphModel):
    ...
    def _fit(self) -> None:
        ...
        ng = gt.Graph(directed=False)
        # gt_g = networkx_to_graphtool(self.input_graph)  # convert to graphtool obj
        ng.add_edge_list(self.input_graph.edges)

        state = gt.minimize_blockmodel_dl(ng)  # run SBM fit
...
# gen_model = BTER(G, 1)
# gen_model._fit()
# gen_model.generate(1,1)
#
# gen_model = ErdosRenyi( G, 1)
# gen_model._fit()
# gen_model.generate(1,1)
#
# gen_model = _BTER(G, 1)
# gen_model._fit()
# gen_model.prep_environment()
# gen_model.generate(1,1)