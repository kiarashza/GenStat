    ...
    for d_i,dataset in enumerate(datasets):
        ...
        for atpt in range(10):
            if not  Benchmark_on_Original_graphs:
                the_setting_report.append(LinkPredictionFramewok.LinkPrediction(datasets_adj[d_i], encoder=encoder,filename=fileName +  encoder + "_" + get_substring_after_last_slash(dataset)))
            else:
                the_setting_report.append(LinkPredictionFramewok.LinkPrediction(dataset, encoder=encoder,filename=fileName+"_"+encoder+"_"+dataset))
