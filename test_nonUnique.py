...
parser.add_argument('-e', dest="epoch_number" , default=100000, help="Number of Epochs", type=int)
parser.add_argument('-v', dest="Vis_step", default=10000, help="model learning rate")
...
parser.add_argument("-desc_fun", dest="desc_fun" , default= [ "in_degree_dist", "HistogramOfCycles","ReachabilityInKsteps","HistogramOfRandomWalks"], help="the descriptor function which should be used: ", nargs='+',)#[ "in_degree_dist", "HistogramOfCycles","HistogramOfRandomWalks","ReachabilityInKsteps"]
...
parser.add_argument('-LDP', dest="LDP" , default=False,type=bool, help="this paprameter indicate if the graph generation should be under LDP, if LDP == True then graph statisics would be in_degree_dist and HistogramOfCycles, we have not implemented the  Random mechanism for the other statistics ")
...
class Binner():
    ...
    def EqualFreqBinner(featureValues, numBins, UpperBound=None,minWidth=.5):
        ...
        for b in range(numBins):
            ...
            numElementsInEachBin = min(int(len(featureValues) / (numBins)),len(featureValues)-1)
    ...
    def ReachabilityInKsteps(self, adj,applyHist=True):
        ...
        for k_step in range(self.steps_of_reachability):
            ...
            if applyHist:
                ...
                re = self.ReachabilityHist[k_step](reached.sum(-1), self.MAx_reachability[k_step],self.min_reach[k_step])
            else:
                re = reached.sum(-1)
            if self.normalized:
                re = re / adj.shape[-1]
...
if LDP==True:
    ...
    for i_ in range(steps_of_reachability):
        bin_center, binwith = _Binner.combineBinner(binners, featureValuse[i_], num_of_bin, UpperBound=SubGraphNodeNum**(i+2) )
